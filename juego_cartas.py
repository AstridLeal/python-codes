...
def obtener_carta(lista_de_cartas):
  ...
  #Función para eliminar y devolver un elemento de una lista, el último elemento de la lista
  return carta
...
def mazo_barajeado(lista_de_cartas):
  ...
  random.shuffle(mazo_de_salida) #Mezcla o barajea los elementos de una lista de forma aleatoria
  return mazo_de_salida

lista_mazo_inicial = [] #Almacena las cartas del mazo

for palo in TUPLA_PALO: #Itera cada elemento de la TUPLA_PALO
  for valor, rango in enumerate(TUPLA_RANGO): #Itera cada elemento de TUPLA_RANGO y le asigna dos valores
    #La función enumerate() se usa para obtener el índice (valor) y el valor de cada elemento en la tupla
